...
export async function walkTransactions(address, { stopWhen, maxPages = 20, filter } = {}) {
  const all = [];
  let pageParams;
  let hitMaxPages = false;
  for (let page = 0; page < maxPages; page++) {
    const res = await fetchTransactions(address, { pageParams, filter });
    const items = res.items || [];
    for (const item of items) {
      all.push(item);
      if (stopWhen && stopWhen(item)) {
...
export async function fetchFirstFunder(address, { maxPages = 8 } = {}) {
  const lower = address.toLowerCase();

  const [nativeItems, tokenItems] = await Promise.all([
    walkTransactions(address, { maxPages, filter: "to" }),
    walkTokenTransfers(address, { maxPages }),
  ]);
