...
`GET /api/scan/start?kind=deposits|mirror|funding&hours=all&...` starts a scan in the background and returns a `jobId` immediately (HTTP 202); `GET /api/scan/status?jobId=...` polls it (`status`: `"running"` | `"done"` | `"error"`, plus `result` once done). This works because the app is a single long-lived Node process on Railway, not a serverless function — the scan keeps running after the "start" response is sent, and the browser just checks back every couple of seconds (`runScanJob()` in `public/app.js`) instead of holding one connection open for however long the scan takes. Job state is in-memory only (`lib/jobs.js`) and is lost on a redeploy/restart. That used to mean losing 100% of an all-time scan's progress if the process died before it finished — see "Crash recovery: checkpoints" below for what changed. The synchronous `GET /api/clusters/*` routes are unchanged and still used for bounded windows, which are fast enough to answer directly. All-time budgets (`ALL_TIME_BUDGET_MS`, `ALL_TIME_MAX_PAGES` in `lib/aggregate.js`) were raised accordingly (10 minutes, 3000 pages) since they're no longer constrained by a platform request timeout — only by not wanting a truly runaway scan.

### Resolved: Ink Explorer rate-limiting (429) on deep pagination

A genuinely long walk (an "all time" scan, or several concurrent per-wallet lookups) runs into Blockscout's own public rate limit ("Too many requests. Increase limits now at https://dev.blockscout.com") — confirmed live on the first real all-time background-job run. This used to throw and kill the whole scan. `lib/inkExplorer.js`'s `get()` now retries on 429 with backoff (honoring `Retry-After` when present), with two policies: a `patient` mode (long backoff, several retries) used only for all-time background jobs, which have no request-timeout pressure, and a `quick` mode (a couple of short retries) for bounded-window scans, so a rate-limit blip can't undo the tight time budgets described above. Threaded via a `patient` option through `walkTokenTransfers`/`walkTransactions`/`fetchFirstFunder`, set to `hours == null` at the call sites in `lib/aggregate.js`.

### Resolved (important): deposits were being attributed to the wrong contract

The first real "all time" run of cluster #1 surfaced something the rate-limit and timeout fixes above didn't cause: 26,226 real deposits, and `uniqueWallets: 1`. That one "wallet" turned out to be Nado's own **Endpoint** contract, not a user.

Root cause: `fetchGlobalDeposits()` was walking incoming USDT0 transfers to the **Clearinghouse** contract and reading the transfer's `from` field as "the depositor." That's wrong for Nado's on-chain flow — deposits go through two on-chain hops, `user → Endpoint → Clearinghouse` (this matches the Vertex-Protocol-family architecture the contract naming already pointed at). The Transfer event landing *in* the Clearinghouse always shows `from = Endpoint`, regardless of which real user initiated it, because Endpoint is the one forwarding the funds along. Confirmed with a raw probe (`GET /api/debug/probe?depositSource=1`, still deployed): incoming Clearinghouse transfers showed exactly 1 distinct `from` address (Endpoint) across 47 transfers in 2 pages, while incoming *Endpoint* transfers showed 36 distinct real addresses in 50 transfers over the same 2 pages.

Fixed by pointing `fetchGlobalDeposits()` and `fetchWalletDeposits()` at `NADO_CONTRACTS.endpoint` instead of `.clearinghouse`. This is the actual, practical answer to "how many Nado wallets are there / what are they": every wallet that has ever deposited USDT0 into Nado via the Endpoint contract, since there's no public "list all accounts" endpoint. This bug predates every other fix in this README — clusters #1 and #3 were silently comparing every depositor against the same fake "wallet" the whole time, however fast or slow the scan ran. If a wallet-attribution result ever again looks suspiciously collapsed (one dominant address, or a cluster that includes what looks like a Nado contract itself), re-run `?depositSource=1` first before assuming it's a real finding.

### Crash recovery: checkpoints survive a restart

All-time scans run for up to an hour (`ALL_TIME_BUDGET_MS`) and hold their growing results only in memory while they run — a live production run showed memory climbing steadily (Railway's Metrics tab: ~350MB → ~600MB over 15 minutes against a 1000MB limit) as the deposit/funder arrays grew, which is a real risk of an OOM kill before a long scan finishes. Previously, that meant losing the *entire* run — the in-memory job just vanishes when the process dies, and a fresh container starts from zero.

`lib/checkpoint.js` fixes the "lose everything" part (not the OOM risk itself — that would need bounding the scan's own memory footprint, which is a separate, bigger change if it keeps happening). Every ~15 seconds, an in-progress all-time scan writes its current progress to a small JSON file in the same persistent-Volume-backed directory as the usage stats (`STATS_DIR`, `data/checkpoint-deposits.json` / `data/checkpoint-funding.json`) — the deposits scan saves the unique wallet list found so far, the funding scan saves the funder→funded edges found so far. This is **not** a resumable scan (it doesn't continue Blockscout's pagination from where it left off — that would mean persisting its opaque cursor across a restart too, more than this needed) — it's a "don't lose the work already done" safety net.

`GET /api/scan/checkpoint?kind=deposits|funding` reads this straight from disk, independent of the in-memory jobs Map — so it still answers even after a crash wiped every in-memory job (`/api/scan/status` would 404 on the old jobId at that point). For `kind=funding` it also recomputes clusters fresh from the saved edges (cheap, pure in-memory grouping) rather than just returning raw counts. Practical effect: if a scan dies mid-run, the next check doesn't have to be "start over from zero, hope it doesn't OOM again this time" — whatever it found up to the last checkpoint (a few seconds before the crash, at most) is still there to read back and report as a clearly-labeled partial result.
...
2. **Exchange wallet exclude list is empty.** `lib/exchangeWallets.js` ships with no real addresses in it. See "Excluding exchange wallets" above for how to fill it in. Now that deposits correctly resolve to real wallets, this matters more — Nado's own Endpoint/Clearinghouse addresses should never show up as depositors themselves, but other infrastructure (a bridge, a market maker topping up many accounts) legitimately might.
