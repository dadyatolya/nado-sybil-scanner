...
// UPDATE (first real production run): this used to be normalizeMatchesResponse(),
// built around a guessed "/matches" + "/txs" response pair (Vertex-family
// shape) that turned out not to exist on Nado's actual archive-indexer.
// The real endpoint is "/orders" — confirmed via docs.nado.xyz — and each
// order record is self-contained (no separate txs array to join against by
// submission_idx). See lib/nadoClient.js's fetchOrders(). One real
// consequence: the old code could pair up direct taker<->maker fills from
// the txs[].tx.match_orders envelope; the real /orders response carries no
// counterparty information at all (each record only reflects the querying
// subaccount's own side), so that "direct counterparty" signal (Signal A in
// findMirrorTradeClusters above) can no longer be produced from this data
// source — only the independent mirrored-open/close signal (Signal B) can.
export function normalizeOrdersResponse(raw, { productLabel } = {}) {
  const orders = raw?.orders || [];
  const fills = [];

  for (const o of orders) {
    const wallet = addressFromSubaccount(o.subaccount);
    if (!wallet) continue;

    const timestamp = Number(o.last_fill_timestamp ?? o.first_fill_timestamp);
    if (!Number.isFinite(timestamp)) continue;

    const product = productLabel ?? o?.pre_balance?.base?.perp?.product_id ?? o?.post_balance?.base?.perp?.product_id ?? o.product_id ?? "unknown";

    const preAmt = x18ToNumber(o?.pre_balance?.base?.perp?.balance?.amount);
    const postAmt = x18ToNumber(o?.post_balance?.base?.perp?.balance?.amount);
    const baseFilled = x18ToNumber(o?.base_filled);
    const evidence = { digest: o.digest, submissionIdx: String(o.submission_idx ?? "") };
