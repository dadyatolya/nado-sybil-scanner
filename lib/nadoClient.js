...
async function rawProbe(method, url, body) {
  const controller = new AbortController();
  const t = setTimeout(() => controller.abort(), 8000);
  try {
    const opts = { method, signal: controller.signal, headers: { "Accept-Encoding": "gzip" } };
    if (body !== undefined) {
    ...
    const text = await res.text();
    return { status: res.status, bodySnippet: text.slice(0, 250) };
  } catch (err) {
...
/** Diagnostic-only: the /orders and /oracle-price 404s (even on the
 * "correct", docs-confirmed unified base) raised the question of whether
 * the whole "archive-indexer v1" family of POST endpoints is actually live
 * anywhere right now, or whether docs.nado.xyz describes a deprecated/
 * not-yet-deployed generation. This hits several base-URL + path
 * combinations at once (including a v2 REST /trades sanity check) and
 * reports raw status codes so a 404 (route doesn't exist) can be told apart
 * from a 400 (route exists, request was just malformed) without guessing
 * one combination per deploy cycle. */
export async function debugMatrixProbe() {
  const bases = {
    archiveV1Unified: "https://api.prod.nado.xyz/archive/v1",
    archiveV1Legacy: "https://archive.prod.nado.xyz/v1",
    archiveV2Unified: "https://api.prod.nado.xyz/archive/v2",
    archiveV2Legacy: "https://archive.prod.nado.xyz/v2",
  };
  const dummySubaccount = `0x${"0".repeat(64)}`;
  const results = {};
  for (const [name, base] of Object.entries(bases)) {
    results[`${name}_oraclePrice`] = await rawProbe("POST", `${base}/oracle-price`, { oracle_price: { product_ids: [1] } });
    results[`${name}_orders`] = await rawProbe("POST", `${base}/orders`, { orders: { product_ids: [1], limit: 2 } });
    results[`${name}_portfolio`] = await rawProbe("POST", `${base}/portfolio`, { portfolio: { subaccount: dummySubaccount } });
  }
  results.v2_trades = await rawProbe("GET", `${bases.archiveV2Unified}/trades?ticker_id=BTC-PERP_USDT0&limit=2`);
  results.v2_symbols = await rawProbe("GET", `${bases.archiveV2Unified}/symbols`);
  return results;
}
