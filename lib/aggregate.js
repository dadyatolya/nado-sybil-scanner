...
/** All incoming USDT0 transfers to Nado's Clearinghouse contract in the last
 * `hours`, i.e. the global deposit feed cluster #1 needs. */
export async function fetchGlobalDeposits({ hours = 24, maxPages = 25, budgetMs = DEFAULT_BUDGET_MS } = {}) {
...
export async function fetchWalletDeposits(address, { hours = 24 * 30, maxPages = 10 } = {}) {
  const sinceTs = Math.floor(Date.now() / 1000) - hours * 3600;
...
/** Global trade tape (fills) across the given products for the last `hours`,
 * i.e. what cluster #2 (mirror trading) scans. */
export async function fetchGlobalFills({ hours = 6, productIds, maxPagesPerProduct = 15, budgetMs = DEFAULT_BUDGET_MS } = {}) {
  const startedAt = Date.now();
  const sinceTs = Math.floor(Date.now() / 1000) - hours * 3600;
