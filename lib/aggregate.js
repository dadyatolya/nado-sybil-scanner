...
// How often an in-progress all-time scan writes its progress to disk (see
// lib/checkpoint.js). Frequent enough that a crash loses at most a few
// seconds of work, infrequent enough not to hammer the disk on a
// fast-paginating run.
...
//
// Raised from an initial 10 minutes: the first real run of cluster #3 (the
// most expensive scan — up to two Explorer lookups per depositor, now also
// patiently retrying through rate-limiting) hit the 10-minute ceiling before
// reaching genesis. The user explicitly wants a genuinely complete history
// scan and is fine waiting, so this now allows up to an hour before giving
// up and reporting a partial result — still not literally unbounded, so a
// pagination bug can't run forever, but long enough that a real depositor
// base of a few thousand wallets should finish inside it.
const ALL_TIME_BUDGET_MS = Number(process.env.ALL_TIME_BUDGET_MS) || 60 * 60_000; // 1 hour
const ALL_TIME_MAX_PAGES = Number(process.env.ALL_TIME_MAX_PAGES) || 8000;
...
/** All incoming USDT0 transfers to Nado's Endpoint contract since `hours`
 * ago (or since genesis if `hours` is null), i.e. the global deposit feed
 * cluster #1 needs.
 *
 * IMPORTANT — this used to walk the *Clearinghouse* contract instead, which
 * looked reasonable (it's the contract that ends up holding user funds) but
 * was wrong: confirmed live (first full "all time" background-job run) that
 * every single incoming Clearinghouse transfer has `from = Endpoint`, not a
 * real user — Nado's on-chain deposit flow is two hops (user → Endpoint →
 * Clearinghouse, matching the Vertex-Protocol-family architecture this app's
 * contract naming already noted), so scanning the Clearinghouse side
 * collapsed every depositor into a single "wallet" (Nado's own Endpoint
 * contract). A quick raw probe of incoming Endpoint transfers showed 36
 * distinct real addresses in just 50 transfers, confirming the first hop is
 * where the actual depositor shows up. Fixed by scanning Endpoint instead —
 * see README's API-availability notes for the full story. */
function depositsFromTransferItems(items, sinceTs) {
  const deposits = [];
  for (const item of items) {
    ...
    if (toAddr !== NADO_CONTRACTS.endpoint) continue; // incoming to Endpoint only — see function doc above
...
export async function fetchGlobalDeposits({
  ...
  // Periodically save progress to disk so a crash mid-scan (Railway's memory
  // limit killing the process being the realistic way that happens on a
  // scan this long-running — see lib/checkpoint.js) doesn't lose the whole
  // run. Only worth the disk writes for genuinely long all-time scans;
  // bounded windows finish fast enough not to need it. Explicitly disabled
  // by fetchFundingFanOut's own internal deposit walk below, so the two
  // scans don't stomp on each other's checkpoint file when both run at once.
  checkpoint = hours == null,
} = {}) {
  ...
  const items = await walkTokenTransfers(NADO_CONTRACTS.endpoint, {
    maxPages,
    ...
    onPage: checkpoint
      ? (allSoFar, page) => {
          ...
          const depositsSoFar = depositsFromTransferItems(allSoFar, sinceTs);
          const wallets = [...new Set(depositsSoFar.map((d) => d.wallet))];
          saveCheckpoint("deposits", {
            status: "running",
            hours,
            pagesWalked: page + 1,
            itemsScanned: allSoFar.length,
            depositsConsidered: depositsSoFar.length,
  ...
  const deposits = depositsFromTransferItems(items, sinceTs);
...
/** Deposits for one specific wallet (still via the same global Endpoint
 ...
 * here for the Checker/Dashboard per-wallet views). Filters for transfers
 * *to* Endpoint, not Clearinghouse — see fetchGlobalDeposits()'s doc comment
 * for why: a user's own deposit transaction sends to Endpoint, which then
 * forwards to Clearinghouse itself, so Clearinghouse never sees the user's
...
export async function fetchWalletDeposits(address, {
  ...
  return items
    .filter((item) => (item.to?.hash || "").toLowerCase() === NADO_CONTRACTS.endpoint)
    .map((item) => ({
...
export function getExcludedFunders() {
...
export async function fetchFundingFanOut({
  ...
  // Same crash-recovery checkpointing as fetchGlobalDeposits (see
  // lib/checkpoint.js) — saved under its own "funding" kind so it doesn't
  // collide with a standalone deposits scan's checkpoint. The internal
  // fetchGlobalDeposits call just below deliberately opts OUT of its own
  // checkpointing (checkpoint: false) so it doesn't overwrite the
  // "deposits" checkpoint with a different scan's numbers.
  checkpoint = hours == null,
} = {}) {
  const startedAt = Date.now();
  ...
  for (let i = 0; i < wallets.length; i += FUNDER_LOOKUP_CONCURRENCY) {
    ...
    if (checkpoint) {
      ...
      if (now - lastCheckpointAt >= CHECKPOINT_INTERVAL_MS) {
        lastCheckpointAt = now;
        saveCheckpoint("funding", {
          status: "running",
          hours,
          minFanOut,
          walletsConsidered: wallets.length,
          walletsScanned: Math.min(i + FUNDER_LOOKUP_CONCURRENCY, wallets.length),
  ...
  if (checkpoint) {
    saveCheckpoint("funding", {
      ...
      minFanOut,
      walletsConsidered: wallets.length,
      walletsScanned: wallets.length - unscanned,
      fundersResolved: fundingEdges.length,
      unresolvedFunders: unresolved,
      edges: fundingEdges,
